#!/usr/bin/env bash
# injection-scan.sh — Codex-runtime PostToolUse hook. Scans the OUTPUT of content-bearing
# tools (Read, WebFetch, Bash, Grep, mcp__*) for prompt-injection markers in untrusted data.
# WARN-ONLY by design: always exits 0, never blocks a tool result.
#
...
#
# WIRING: point your Codex hook config's PostToolUse (or equivalent) entry at this script.
set -uo pipefail
